...
export const Freelancers: React.FC<FreelancersProps> = (props) => {
            ...
            if (formMode === 'add') {
                ...
                try {
                    ...
                    if (formData.name !== selectedMember.name) {
                        setProjects(prevProjects => prevProjects.map((proj: Project) => ({
                            ...proj,
                            team: proj.team.map((t: AssignedTeamMember) => t.memberId === selectedMember.id ? { ...t, name: formData.name } : t)
                        })));
                        setTeamProjectPayments(prevPayments => prevPayments.map((p: TeamProjectPayment) => p.teamMemberId === selectedMember.id ? { ...p, teamMemberName: formData.name } : p));
                    }
    ...
    const handleDelete = async (memberId: string) => {
        if (teamProjectPayments.some(p => p.teamMemberId === memberId && p.status === PaymentStatus.BELUM_BAYAR)) {
        ...
        try {
            ...
            setProjects(prevProjects => prevProjects.map((p: Project) => ({
                ...p,
                team: p.team.filter((t: AssignedTeamMember) => t.memberId !== memberId)
            })));
            // Remove related data
    ...
    const handlePay = async () => {
        ...
        setTeamProjectPayments(prev => prev.map(p => projectsToPay.includes(p.id) ? { ...p, status: PaymentStatus.LUNAS } : p));
        ...
        try {
            ...
            for (const pay of selectedPayments) {
                ...
                if (isFullyPaid) {
                    await markTeamPaymentStatus(pay.id, PaymentStatus.LUNAS);
                } else {
                    await updateTeamPaymentFee(pay.id, pay.fee - payForThisProject, PaymentStatus.BELUM_BAYAR);
                }
            }
    ...
    const selectedMemberUnpaidProjects = useMemo(() => {
        if (!selectedMember) return [];
        return teamProjectPaymentsInDateRange.filter(p => p.teamMemberId === selectedMember.id && p.status === PaymentStatus.BELUM_BAYAR);
    ...
    const handleDeleteNote = async (noteId: string) => {
        if (!selectedMember) return;
        const updatedNotes = selectedMember.performanceNotes.filter((n: PerformanceNote) => n.id !== noteId);
        try {
            const updated = await updateTeamMemberRow(selectedMember.id, { performanceNotes: updatedNotes });
            setTeamMembers(prev => prev.map((m: TeamMember) => m.id === selectedMember.id ? updated : m));
            setSelectedMember(updated);
        } catch (err: any) {
    ...
    const handleSaveSignature = (signatureDataUrl: string) => {
        if (paymentSlipToView) {
            ...
            setPaymentSlipToView((prev: TeamPaymentRecord | null) => prev ? { ...prev, vendorSignature: signatureDataUrl } : null);
        }
                                        ...
                                        <tr key={p.id} className="hover:bg-slate-50/50 transition-colors">
                                            ...
                                            <td className="px-5 py-5 text-center">
                                                <span className="inline-block px-2 py-1 bg-slate-100 rounded text-[10px] font-bold text-slate-600 uppercase">
                                                    {project?.team.find((t: AssignedTeamMember) => t.memberId === member.id)?.role || member.role}
                                                </span>
                                            </td>
    ...
    const handleDownloadTeam = () => {
        ...
        const data = teamMembers.map(member => {
            const unpaidFee = teamProjectPaymentsInDateRange
                .filter(p => p.teamMemberId === member.id && p.status === PaymentStatus.BELUM_BAYAR)
                .reduce((sum, p) => sum + p.fee, 0);
            return [
                    ...
                    <div className="bg-brand-surface/80 backdrop-blur-xl p-6 rounded-2xl shadow-xl border border-white/10 transition-all duration-300">
                        <div className="md:hidden space-y-3">
                            {uniqueTeamMembers.map(member => {
                                const unpaidFee = teamProjectPaymentsInDateRange.filter(p => p.teamMemberId === member.id && p.status === PaymentStatus.BELUM_BAYAR).reduce((sum, p) => sum + p.fee, 0);
                ...
                <div className="flex flex-col h-full">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6 non-printable">
                        <div className="bg-white/5 backdrop-blur-md p-4 rounded-2xl border border-white/10 shadow-lg">
                            ...
                            <p className="text-2xl font-bold text-gradient">
                                {teamProjectPayments.filter(p => p.teamMemberId === selectedMember.id && p.status === PaymentStatus.LUNAS).length}
                            </p>
                        ...
                        <div className="bg-white/5 backdrop-blur-md p-4 rounded-2xl border border-white/10 shadow-lg">
                            ...
                            <p className="text-2xl font-bold text-gradient">
                                {formatCurrency(teamProjectPayments.filter(p => p.teamMemberId === selectedMember.id && p.status === PaymentStatus.LUNAS).reduce((sum, p) => sum + p.fee, 0))}
                            </p>
                    ...
                    <div className="pt-0 md:pt-5 max-h-[65vh] overflow-y-auto pr-2 pb-4">
                        {detailTab === 'projects' && (
                            <div className="space-y-6">
                                <div className="flex justify-between items-center">
                                    <h4 className="text-sm md:text-base font-semibold text-brand-text-light">Acara Pernikahan Terkait</h4>
                                    {selectedMemberUnpaidProjects.length > 0 && (
                                        <button onClick={handleCreatePayment} className="button-primary !py-2 !px-4 text-xs">Lanjutkan ke Pembayaran</button>
                                    )}
                                </div>
                                <FreelancerProjects 
                                    projects={projects} 
                                    ...
                                    onToggleProject={(paymentId) => setProjectsToPay(prev => prev.includes(paymentId) ? prev.filter(i => i !== paymentId) : [...prev, paymentId])}
                                    showOnlyUnpaid={true}
                                />
                            </div>
                        )}
                                        ...
                                        {expandedRecordId === record.id && (
                                            <div className="mt-3 bg-brand-bg rounded-lg p-3">
                                                ...
                                                <ul className="list-disc list-inside text-sm space-y-1 pl-4">
                                                    {record.projectPaymentIds.map((paymentId: string) => {
                                                        const payment = teamProjectPayments.find(p => p.id === paymentId);
                                                        const project = projects.find((p: Project) => p.id === payment?.projectId);
                                                        return (
                        ...
                        {detailTab === 'create-payment' && selectedMember && (
                            <CreatePaymentTab
                                ...
                                renderPaymentDetailsContent={() => renderPaymentSlipBody({ 
                                    ...
                                    items: selectedMemberUnpaidProjects.filter(p => projectsToPay.includes(p.id)).map((p: TeamProjectPayment) => {
                                        const prj = projects.find(proj => proj.id === p.projectId);
                                        return {
                                            projectId: p.projectId,
                                            projectName: prj?.projectName || 'Acara Pernikahan',
                                            amount: p.fee,
                                            projectDate: prj?.date || new Date().toISOString()
                                        };
                                    })
                                })}
                                cards={cards}
